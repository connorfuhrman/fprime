...
// Ensure that jluna is always initialized before
// the execution of any Fprime executable. jluna
// will throw an exception if you link to the library
// and don't initialize
void before_main() __attribute__((constructor (101)));
void before_main()
{
  jluna::initialize(get_nprocs_conf());
}
...
namespace Os {
    ...
    Task::TaskStatus create_pthread(NATIVE_UINT_TYPE, NATIVE_UINT_TYPE, NATIVE_UINT_TYPE, pthread_t*& tid, void* arg, bool) {
	// Create wrapper lambda around the entry wrapper
	auto entry_lambda = [](void* arg) -> void {
	  pthread_entry_wrapper(arg);
	};

	// Register the task with the jluna threadpool
	
	auto _jl_task = jluna::ThreadPool::create<void(void*)>(entry_lambda, arg);
	// Run the task
	_jl_task.schedule();

	// Store the task handle in the tid
	auto jl_task = new jluna::Task<void>(std::move(_jl_task));
	tid = reinterpret_cast<pthread_t*>(jl_task);
    ...
    Task::~Task() {
    	if (this->m_handle) {
	  delete reinterpret_cast<jluna::Task<void>*>(this->m_handle);
    	}
	...
	auto task = reinterpret_cast<jluna::Task<void>*>(this->m_handle);
	task -> join();
